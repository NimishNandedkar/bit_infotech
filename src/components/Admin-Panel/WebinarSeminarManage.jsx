...
const WebinarSeminarManage = () => {
  ...
  const getRegisteredUsers = async (projectId) => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/webinars/getRegisteredUsers/${projectId}`);
      console.log(response.data.data);
              ...
              {projects.map(project => (
                <TableRow key={project._id}>
                  ...
                  <TableCell>
                    ...
                    <Button variant="contained" color="primary" style={{ marginLeft: '8px' }} onClick={() => getRegisteredUsers(project._id)}>Registered users</Button>
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
